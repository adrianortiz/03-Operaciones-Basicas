import java.awt.Container;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;

import javax.swing.JButton;
import javax.swing.JFrame;
import javax.swing.JLabel;
import javax.swing.JTextField;

/**
 * 
 * @author Adrian
 *
 */
public class Ventana extends JFrame implements ActionListener{
	
	private static final long serialVersionUID = 1L;
	
	private JTextField txtN1 = new JTextField();
	private JTextField txtN2 = new JTextField();
	
	private JLabel lbN1 = new JLabel("Número 1");
	private JLabel lbN2 = new JLabel("Número 2");
	private JLabel lbResultado = new JLabel("Resultado");
	
	private JButton btnCalcular = new JButton("Calcular");
	
	// Obtener container directamente de la ventana
	private Container c = getContentPane();
	
	private Operaciones op = new Operaciones();
	
	/**
	 * Constructor que inicia la ventana y carga los controles por defecto
	 */
	public Ventana(){
		super.setTitle("Operaciones");
		super.setSize(320, 480);
		super.setDefaultCloseOperation(EXIT_ON_CLOSE);
		
		cargarControles();
	}

	/**
	 * Cargar todos los controles
	 */
	private void cargarControles() {
		
		// No usar un Layout para acomodar mis controles
		c.setLayout(null);
		
		lbN1.setBounds(10, 10, 300, 30);
		txtN1.setBounds(10, 40, 300, 30);
		
		lbN2.setBounds(10, 80, 300, 30);
		txtN2.setBounds(10, 110, 300, 30);
		
		btnCalcular.setBounds(10, 150, 300, 30);
		
		lbResultado.setBounds(10, 190, 300, 30);
		
		c.add(lbN1);
		c.add(txtN1);
		
		c.add(lbN2);
		c.add(txtN2);
		
		c.add(btnCalcular);
		c.add(lbResultado);
		
		// Asignar un ActionListener a un elemento de la interfaz
		btnCalcular.addActionListener(this);
	}

	/**
	 * 
	 */
	@Override
	public void actionPerformed(ActionEvent e) {
		
		int r = op.sumar(Integer.parseInt(txtN1.getText()),Integer.parseInt(txtN2.getText()));
		String myString = String.format("%s + %s = %d", txtN1.getText(), txtN2.getText(), r);
		lbResultado.setText(myString);
	}
	
}
